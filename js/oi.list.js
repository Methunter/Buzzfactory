...
angular.module('oi.list', [])
	...
	.factory('oiList', 
		['oiListCache', 
			function (oiListCache) {
				return function (scope, url, Resource, generate, options) {

					if (typeof oiListCache.cacheRes[url] === 'undefined') {
						oiListCache.cacheRes[url] = [];
					}
					var dataRes = oiListCache.cacheRes[url],
						...
						if (angular.equals(dataRes, [])) {
							scope.items = dataRes = oiListCache.cacheRes[url] = Resource.query( 
								function(data){
							...
							if (options.preloader !== undefined){
								dataRes.push(options.preloader)
							} else {
								scope.items = dataRes;
							}
							dataRes.resName = url;
